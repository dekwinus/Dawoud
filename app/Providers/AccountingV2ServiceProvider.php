        ...
        try {
            ...
            if (class_exists(\App\Models\PaymentPurchase::class)) {
                \App\Models\PaymentPurchase::created(function ($payment) {
                    try {
                        // Only fire accounting event if payment is already approved (skip pending)
                        if (($payment->approval_status ?? 'pending') === 'approved') {
                ...
                \App\Models\PaymentPurchase::updated(function ($payment) {
                    try {
                        // Fire accounting entry when payment is approved
                        if (method_exists($payment, 'wasChanged') && $payment->wasChanged('approval_status') && $payment->approval_status === 'approved') {
                            event(new \App\Events\PaymentPurchaseCreated($payment));
                        }
                        // Fire reversal when soft-deleted
                        if (method_exists($payment, 'wasChanged') && $payment->wasChanged('deleted_at') && ! is_null($payment->deleted_at)) {
