...
class PaymentPurchasesController extends BaseController
{
    ...
    public function index(request $request)
    {
        ...
        foreach ($Payments as $Payment) {
            ...
            $item['approval_status'] = $Payment->approval_status ?? 'pending';
            $data[] = $item;
        }
    ...
    public function store(Request $request)
    {
        ...
        if ($request['montant'] > 0) {
            \DB::transaction(function () use ($request) {
                ...
                // Create payment as pending — account balance & purchase totals updated on approval
        ...
        return response()->json(['success' => true, 'message' => 'Payment created successfully (pending approval)'], 200);
    }

    // ----------- Approve Payment Purchase (Admin) --------------\\

    public function approve(Request $request, $id)
    {
        $this->authorizeForUser($request->user('api'), 'payment_purchases_edit', PaymentPurchase::class);
    ...
    public function update(Request $request, $id)
    {
        ...
        \DB::transaction(function () use ($id, $request) {
            ...
            $wasApproved = $payment->approval_status === 'approved';
            $purchase = Purchase::whereId($request['purchase_id'])->first();

            // Only reverse old balance/totals if the payment was already approved
            if ($wasApproved) {
                ...
                if ($newAccount) {
                    $newAccount->update(['balance' => $newAccount->balance - $request['montant']]);
                }
    ...
    public function destroy(Request $request, $id)
    {
        ...
        \DB::transaction(function () use ($id, $request) {
            ...
            // Only reverse account balance and purchase totals if payment was approved
            if ($wasApproved) {
