...
class ReportController extends BaseController
{
    ...
    public function Report_Sales(request $request)
    {
        ...
        // Search With Multiple Param
            ->where(function ($query) use ($request) {
             ...
             // Filter by "due_soon" (unpaid and older than 14 days)
             ->when($request->filled('filter') && $request->filter == 'due_soon', function ($query) {
                return $query->where('payment_statut', '!=', 'paid')
                             ->whereDate('date', '<=', \Carbon\Carbon::now()->subDays(14));
            });
    ...
    {
        // 1. Quantity Alerts (existing logic)
        $quantity_alert = product_warehouse::join('products', 'product_warehouse.product_id', '=', 'products.id')
            ->whereRaw('qte <= stock_alert')
            ->where('product_warehouse.deleted_at', '=', null)
            ->count();

        // 2. Invoice Due Soon (Unpaid for 14 days)
        // payment_statut != 'paid' implies 'unpaid' or 'partial'
        $invoice_due_alert = Sale::where('deleted_at', '=', null)
            ->where('payment_statut', '!=', 'paid')
            ->whereDate('date', '<=', Carbon::now()->subDays(14))
            ->count();

        // 3. Expired Soon (Expiry date within 30 days)
        // We only care about stock that is actually present (qte > 0)
        $expiry_alert = product_warehouse::join('products', 'product_warehouse.product_id', '=', 'products.id')
            ->where('product_warehouse.deleted_at', '=', null)
            ->where('product_warehouse.qte', '>', 0)
            ->whereNotNull('products.expiry_date')
            ->whereDate('products.expiry_date', '<=', Carbon::now()->addDays(30))
            ->whereDate('products.expiry_date', '>=', Carbon::now()) // Optional: Don't count already expired? Or do we? User said "Expired soon", usually implies future or immediate past. Let's stick to "will in 30 days" which usually means <= today+30.
            ->count();

        // If you want to include already expired in "Expired soon" (e.g. expired yesterday is also urgent), remove the >= Carbon::now() check.
        // For "Expired soon", usually means approaching consumption limit.
        // Let's assume <= Now + 30 days covers everything including past expired items still in stock.

        return response()->json([
            'quantity_alert' => $quantity_alert,
            'invoice_due_alert' => $invoice_due_alert,
    ...
    public function report_top_products(request $request)
    {
        ...
        $products_data = SaleDetail::join('sales', 'sale_details.sale_id', '=', 'sales.id')
            ...
            ->select(
                ...
                DB::raw('sum(sale_details.total - (sale_details.quantity * products.cost)) as profit'),
            )
    ...
    public function seller_report(Request $request)
    {
        ...
        foreach ($users as $user) {
            ...
            // Fetch sums in one query
            $sums = $salesQuery->select(
            ...
            $row = [
                ...
                'total_sales' => number_format($sums->total_sales, 2, '.', ','),
                'total_paid' => number_format($sums->total_paid, 2, '.', ','),
            ];
    ...
    {
        $this->authorizeForUser($request->user('api'), 'Reports_quantity_alerts', Product::class);
        ...
        // Check view records permission
        $ShowRecord = $user->hasRecordView();

        // 3. Expiry Alert Logic (must match UserController logic)
        $products = product_warehouse::join('products', 'product_warehouse.product_id', '=', 'products.id')
            ->join('warehouses', 'product_warehouse.warehouse_id', '=', 'warehouses.id')
            ->where('product_warehouse.deleted_at', '=', null)
            ->where('product_warehouse.qte', '>', 0)
            ->whereNotNull('products.expiry_date')
            // Expiring in next 30 days OR already expired
            ...
            ->select(
                ...
                DB::raw('DATEDIFF(products.expiry_date, NOW()) as days_left')
            )
            ->paginate($limit);
