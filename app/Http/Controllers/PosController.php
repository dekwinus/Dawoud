...
class PosController extends BaseController
{
    ...
    public function CreatePOS(Request $request)
    {
        ...
        if ($client->credit_limit > 0) {
            ...
            $newDue = $request->GrandTotal - $totalPaidAdjusted;
            $newTotalDebt = $client->net_balance + $newDue;

            if ($newTotalDebt > $client->credit_limit) {
                return response()->json([
                    'message' => 'Credit Limit Exceeded',
                    'exceeded_amount' => $newTotalDebt - $client->credit_limit,
                ], 422);
            }
        }
        ...
        try {
            $sale = \DB::transaction(function () use ($request, $totalPaid, $saleUuid) {
                ...
                $order->statut = $isAdmin ? 'completed' : 'pending';
                $order->approval_status = $isAdmin ? 'approved' : 'pending';
                ...
                foreach ($data as $key => $value) {
                    ...
                    // For non-admin (pending approval), skip stock deduction — it will happen on approval.
                    if ($isAdmin && ! $isService && $unit) {
                ...
                // For non-admin (pending approval): create PaymentSale records with status='pending'
                // and do NOT update account balances. On approval, the SalesApprovalController will process.
                foreach ($request->payments as $index => $payment) {
                    if (isset($payment['amount']) && $payment['amount'] > 0) {
                        // Dynamic Accounting: Auto-link to account from PaymentMethod
                        $pmModel = \App\Models\PaymentMethod::find($payment['payment_method_id']);
                        $accountId = $pmModel && $pmModel->account_id ? $pmModel->account_id : ($payment['account_id'] ?? null);
                        ...
                        // Stripe processing only for admin (approved) sales
                        // NOTE: ID 1 is now 'Inistapay', manual payment. Logic disabled.
                        if ($isAdmin && false) { // Previously: ($payment['payment_method_id'] == 1 || ...)
                        ...
                        $paymentSale = PaymentSale::create([
                            ...
                            'approved_by' => $isAdmin ? Auth::user()->id : null,
                        ...
                        // Only update account balance for admin (approved) sales
                ...
                // 🪙 Points logic — only for admin (approved) sales
                $order_used_points = 0;
                ...
                $order->update([
                    ...
                    'paid_amount' => $isAdmin ? $totalPaidAdjusted : 0,
                    'payment_statut' => $isAdmin ? ($due <= 0 ? 'paid' : ($due < $order->GrandTotal ? 'partial' : 'unpaid')) : 'unpaid',
                ]);
        ...
        if ($draft) {
            ...
            $sale = \DB::transaction(function () use ($request, $draft) {
                ...
                try {
                    ...
                    if ($request['amount'] > 0) {
                        if (false) { // Previously: ($request->payment['payment_method_id'] == 1 ...
                            ...
                            // Paying Method Cash (and all non-Stripe methods)
                        } else {

                            // Dynamic Accounting: Auto-link to account
                            $pm_id = $request->payment['payment_method_id'];
                            $pmModel = \App\Models\PaymentMethod::find($pm_id);
                            $resolved_account_id = $pmModel ? $pmModel->account_id : ($request->payment['account_id'] ?? null);
                            ...
                            $account = Account::find($resolved_account_id);
    ...
    public function GetProductsByParametre(request $request)
    {
        ...
        foreach ($product_warehouse_data as $product_warehouse) {
            ...
            $item['expiry_date'] = $product_warehouse['product']->expiry_date;
