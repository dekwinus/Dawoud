  ...
  <div class="vertical-top-nav">
    ...
    <div class="nav-right">
      ...
      <div class="dropdown">
        ...
        >
          <template slot="button-content">
            <span class="badge badge-primary" v-if="notifs_total > 0">{{ notifs_total }}</span>
            <i class="i-Bell"></i>
          </template>
          <vue-perfect-scrollbar
            :settings="{ suppressScrollX: true, wheelPropagation: false }"
          ...
          >
            <!-- Quantity Alerts -->
            <div class="notification-item" v-if="count_quantity_alert > 0">
              ...
              <div class="notif-content" v-if="currentUserPermissions && currentUserPermissions.includes('Reports_quantity_alerts')">
                <router-link tag="a" to="/app/reports/quantity_alerts">
                  <p>{{ count_quantity_alert }} {{ $t('ProductQuantityAlerts') }}</p>
                </router-link>
              </div>
            </div>

            <!-- Invoice Due Alerts -->
            <div class="notification-item" v-if="count_invoice_due_alert > 0">
              <div class="notif-icon">
                <i class="i-Billing text-danger"></i>
              </div>
              <div class="notif-content">
                  <router-link tag="a" to="/app/reports/sales_report?filter=due_soon">
                  <p>{{ count_invoice_due_alert }} Invoice Due Soon</p>
                  </router-link>
              </div>
            </div>

            <!-- Expiry Alerts -->
            <div class="notification-item" v-if="count_expiry_alert > 0">
              <div class="notif-icon">
                <i class="i-Calendar text-warning"></i>
              </div>
              <div class="notif-content">
                  <router-link tag="a" to="/app/reports/expiry_alerts">
                  <p>{{ count_expiry_alert }} Products Expiring Soon</p>
                  </router-link>
              </div>
            </div>

             <!-- Empty State -->
            <div class="notification-item" v-if="notifs_total == 0">
...
export default {
  ...
  computed: {
    ...mapGetters([
      ...
      "notifs_total",
      "count_quantity_alert",
      "count_invoice_due_alert",
      "count_expiry_alert",
      "show_language",
