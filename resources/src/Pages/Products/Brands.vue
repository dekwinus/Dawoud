...
const loadRows = async () => {
  loading.value = true;
  try {
    const { data } = await axios.get('/api/brands', { params: filters.value });
...
const submit = async () => {
  ...
  try {
    if (editingId.value) {
      await axios.put(`/api/brands/${editingId.value}`, form.value);
    } else {
...
<template>
  <AdminLayout>
    <Head title="العلامات التجارية" />
    <div class="p-6 space-y-4" dir="rtl">
      <div class="flex items-center justify-between gap-3">
        <h1 class="text-2xl font-bold">العلامات التجارية</h1>
        <button @click="openCreate" class="px-4 py-2 rounded-xl bg-[#04724D] text-white text-sm font-bold">إضافة علامة</button>
      </div>
      ...
      <div v-if="showForm" class="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl w-full max-w-lg p-6 space-y-4">
          <h2 class="text-lg font-bold">{{ editingId ? 'تعديل العلامة' : 'إضافة علامة' }}</h2>
          <div class="space-y-2">
