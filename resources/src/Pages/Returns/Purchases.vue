...
const fetchReturns = async () => {
  loading.value = true;
  try {
    const { data } = await axios.get('/api/returns/purchase', { params: filters.value });
    returns.value = data.purchase_returns || [];
...
const openDetails = async (r) => {
  ...
  try {
    const { data } = await axios.get(`/api/returns/purchase/${r.id}`);
...
const deleteReturn = async (id) => {
  ...
  await axios.delete(`/api/returns/purchase/${id}`);
  fetchReturns();
};
...
<template>
  <AdminLayout>
    ...
    <div class="space-y-8 p-6 md:p-10 font-['Cairo']" dir="rtl">
      <div>
        <div class="flex items-center gap-2 text-blue-500 text-xs font-black uppercase tracking-widest mb-1"><RotateCcw class="w-4 h-4" />مرتجعات المشتريات</div>
        <h1 class="text-4xl font-black text-gray-900 dark:text-white">مرتجعات المشتريات</h1>
      </div>
      ...
      <div class="bg-white dark:bg-[#0A0A0A] rounded-[40px] border border-gray-100 dark:border-gray-800 overflow-hidden">
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-right">
            <thead><tr class="text-[10px] font-black text-gray-400 uppercase tracking-widest bg-gray-50/50 dark:bg-white/[0.01]">
              <th class="px-8 py-6">التاريخ</th><th class="px-6 py-6">المرجع</th><th class="px-6 py-6">المورد</th><th class="px-6 py-6">الإجمالي</th><th class="px-6 py-6">إجراءات</th>
            </tr></thead>
            <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
              ...
              <tr v-else-if="returns.length===0"><td colspan="5" class="py-20 text-center text-gray-400 font-black text-sm">لا توجد مرتجعات</td></tr>
              <tr v-for="r in returns" :key="r.id" class="group hover:bg-gray-50/50 dark:hover:bg-white/[0.01] transition-all">
                ...
                <td class="px-6 py-5 font-mono font-black text-blue-500">{{ r.Ref }}</td>
                <td class="px-6 py-5 text-sm font-black text-gray-700 dark:text-gray-300">{{ r.provider_name || r.supplier_name || '—' }}</td>
                ...
                <td class="px-6 py-5">
                  <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all">
                    ...
                    <a :href="`/purchase_return_pdf/${r.id}`" target="_blank" class="p-3 bg-white dark:bg-[#1A1A1A] text-gray-400 hover:text-blue-500 rounded-2xl border border-gray-100 dark:border-gray-800 transition-all flex"><Printer class="h-4 w-4" /></a>
        ...
        <div class="p-8 bg-gray-50/50 dark:bg-white/[0.02] border-t border-gray-100 dark:border-gray-800 flex items-center justify-between">
          <p class="text-xs font-black text-gray-400">الإجمالي: <span class="text-blue-500">{{ totalRows }}</span></p>
    ...
    <div v-if="showDetails" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div class="bg-white dark:bg-[#121212] rounded-[32px] w-full max-w-2xl p-8 shadow-2xl max-h-[90vh] overflow-y-auto space-y-4">
        <div class="flex items-center justify-between">
          <div><h2 class="text-xl font-black text-gray-900 dark:text-white">تفاصيل مرتجع المشتريات</h2><p class="text-xs font-mono text-blue-500 mt-1">{{ selectedReturn?.Ref }}</p></div>
        ...
        <div v-else class="border border-gray-100 dark:border-gray-800 rounded-2xl overflow-hidden">
          <table class="w-full text-sm">
            <thead class="bg-gray-50 dark:bg-white/5"><tr class="text-[10px] font-black text-gray-400 uppercase tracking-widest"><th class="px-4 py-3 text-right">المنتج</th><th class="px-4 py-3 text-right">الكمية</th><th class="px-4 py-3 text-right">التكلفة</th></tr></thead>
            <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
              ...
              <tr v-for="(d,i) in returnDetails" :key="i">
                ...
                <td class="px-4 py-3 font-mono font-black dark:text-gray-200">{{ Number(d.cost||d.Unit_cost||0).toLocaleString('ar-EG') }}</td>
              </tr>
            </tbody>
          </table>
        </div>
