...
const loadData = async () => {
  loading.value = true;
  try {
    const response = await axios.get('/api/accounting/v2/reports/balance-sheet', {
      params: {
        page: page.value,
        limit: perPage.value,
        search: search.value,
    ...
    summary.value = response.data.summary || { assets: 0, liabilities: 0, equity: 0 };
...
<template>
  <AdminLayout>
    <Head title="الميزانية العمومية - DawPOS" />
    
    <div class="p-8 space-y-6" dir="rtl">
      <div class="flex items-center justify-between mb-6">
        <div>
          <h1 class="text-2xl font-bold text-gray-900 dark:text-white font-['Cairo']">الميزانية العمومية</h1>
          <p class="text-gray-500 dark:text-gray-400 text-sm font-['Cairo']">عرض الأصول والخصوم وحقوق الملك</p>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl p-6 text-white">
          <div class="flex items-center justify-between mb-2">
            <h3 class="text-lg font-bold font-['Cairo']">الأصول</h3>
            <BalanceIcon class="h-6 w-6 opacity-80" />
          </div>
          <p class="text-3xl font-black font-['Cairo']">{{ summary.assets }}</p>
        </div>
        <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-2xl p-6 text-white">
          <div class="flex items-center justify-between mb-2">
            <h3 class="text-lg font-bold font-['Cairo']">الخصوم</h3>
            <BalanceIcon class="h-6 w-6 opacity-80" />
          </div>
          <p class="text-3xl font-black font-['Cairo']">{{ summary.liabilities }}</p>
        </div>
        <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-2xl p-6 text-white">
          <div class="flex items-center justify-between mb-2">
            <h3 class="text-lg font-bold font-['Cairo']">حقوق الملك</h3>
            <BalanceIcon class="h-6 w-6 opacity-80" />
          </div>
          <p class="text-3xl font-black font-['Cairo']">{{ summary.equity }}</p>
        </div>
      </div>
          ...
          <table class="w-full text-sm">
            <thead class="bg-gray-50 dark:bg-white/5">
              <tr>
                ...
                <th class="p-4 text-right font-bold text-gray-700 dark:text-gray-300 font-['Cairo']">الرصيد</th>
              </tr>
            </thead>
            <tbody>
              <tr v-if="loading">
                <td colspan="4" class="p-8 text-center text-gray-500">جاري التحميل...</td>
              </tr>
              <tr v-else-if="data.length === 0">
                <td colspan="4" class="p-8 text-center text-gray-500">لا توجد بيانات</td>
              </tr>
              <tr v-for="row in data" :key="row.id" class="border-t dark:border-gray-800">
                ...
                <td class="p-4">
                  <span 
                    :class="{
                      'bg-blue-100 text-blue-700': row.type === 'asset',
                      'bg-red-100 text-red-700': row.type === 'liability',
                      'bg-green-100 text-green-700': row.type === 'equity'
                    }"
