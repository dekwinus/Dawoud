...
import { ShoppingCart, Search, Eye, Edit } from 'lucide-vue-next';

const orders = ref([]);
const totalRows = ref(0);
const loading = ref(false);
const filters = ref({ search: '', status: '', page: 1, limit: 15 });
...
const fetchOrders = async () => {
  ...
  try {
    ...
    totalRows.value = data.total || data.totalRows || 0;
...
const viewOrder = async (id) => {
  ...
  } catch (e) { alert('فشل تحميل الطلب'); }
};

const updateStatus = async (id, status) => {
  try {
    await axios.patch(`/api/store/orders/${id}`, { status });
    fetchOrders();
  } catch (e) { alert('فشل تحديث الحالة'); }
};

const statusClass = (s) => ({ pending: 'bg-orange-100 text-orange-700', processing: 'bg-blue-100 text-blue-700', completed: 'bg-green-100 text-green-700', cancelled: 'bg-red-100 text-red-600' }[s] || 'bg-gray-100 text-gray-600');
const statusLabel = (s) => ({ pending: 'معلق', processing: 'جاري المعالجة', completed: 'مكتمل', cancelled: 'ملغي' }[s] || s);
  ...
  <AdminLayout>
      ...
      <div class="bg-white dark:bg-[#0A0A0A] rounded-[40px] border border-gray-100 dark:border-gray-800 overflow-hidden">
        <div class="p-8 border-b border-gray-100 dark:border-gray-800 flex flex-wrap gap-4">
          <div class="relative flex-1 min-w-[260px]">
            ...
            <input v-model="filters.search" @input="filters.page=1; fetchOrders()" placeholder="بحث..." class="w-full pr-12 pl-4 py-4 bg-gray-50 dark:bg-white/5 border-none rounded-2xl text-sm font-black" />
          </div>
          <select v-model="filters.status" @change="filters.page=1; fetchOrders()" class="bg-gray-50 dark:bg-white/5 border-none rounded-2xl px-4 py-4 text-sm font-black text-gray-700 dark:text-gray-300">
            ...
            <option value="pending">معلق</option><option value="processing">جاري المعالجة</option><option value="completed">مكتمل</option><option value="cancelled">ملغي</option>
          </select>
        </div>
          ...
          <table class="w-full text-right">
            ...
            <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
              ...
              <tr v-for="o in orders" :key="o.id" class="group hover:bg-gray-50/50 dark:hover:bg-white/[0.01] transition-all">
                <td class="px-8 py-5 font-black text-gray-900 dark:text-white text-sm">#{{ o.id }}</td>
                ...
                <td class="px-6 py-5">
                  <div class="flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
                    <button @click="viewOrder(o.id)" class="p-3 bg-white dark:bg-[#1A1A1A] text-gray-400 hover:text-[#04724D] rounded-2xl border border-gray-100 dark:border-gray-800"><Eye class="h-4 w-4" /></button>
                    <select v-model="o.status" @change="updateStatus(o.id, o.status)" class="text-xs font-black bg-gray-50 dark:bg-white/5 border-none rounded-xl px-2 py-2">
                      <option value="pending">معلق</option><option value="processing">جاري</option><option value="completed">مكتمل</option><option value="cancelled">ملغي</option>
                    </select>
                  </div>
                </td>
              </tr>
        ...
        <div class="p-8 bg-gray-50/50 dark:bg-white/[0.02] border-t border-gray-100 dark:border-gray-800 flex items-center justify-between">
          ...
          <div class="flex gap-4">
            <button @click="filters.page--; fetchOrders()" :disabled="filters.page===1" class="text-xs font-black text-[#04724D] dark:text-[#3EFF8B] disabled:opacity-20">السابق</button>
            <button @click="filters.page++; fetchOrders()" :disabled="orders.length < filters.limit" class="text-xs font-black text-[#04724D] dark:text-[#3EFF8B] disabled:opacity-20">التالي</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="showDetails" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div class="bg-white dark:bg-[#121212] rounded-[32px] w-full max-w-lg p-8 space-y-5 shadow-2xl max-h-[90vh] overflow-y-auto">
        <h2 class="text-xl font-black text-gray-900 dark:text-white">تفاصيل الطلب #{{ selectedOrder?.id }}</h2>
        <div v-if="selectedOrder" class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            ...
            <div class="bg-gray-50 dark:bg-white/5 rounded-2xl p-4"><p class="text-xs text-gray-400 font-black">التاريخ</p><p class="font-black text-gray-700 dark:text-gray-300">{{ selectedOrder.created_at }}</p></div>
          </div>
          <div class="bg-gray-50 dark:bg-white/5 rounded-2xl p-4">
            ...
            <div v-if="selectedOrder.items && selectedOrder.items.length" class="space-y-2">
              <div v-for="item in selectedOrder.items" :key="item.id" class="flex justify-between text-sm">
                <span class="text-gray-700 dark:text-gray-300">{{ item.product_name }} × {{ item.quantity }}</span>
                <span class="font-black text-gray-900 dark:text-white">{{ Number(item.price).toLocaleString('ar-EG') }}</span>
              </div>
            </div>
