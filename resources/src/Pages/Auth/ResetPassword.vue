  ...
  <div dir="rtl" class="min-h-screen flex bg-white font-sans selection:bg-[#3EFF8B]/30 tracking-tight">
    ...
    <div class="hidden lg:flex lg:w-1/2 bg-[#04724D] relative overflow-hidden flex-col justify-between p-16 order-2">
      ...
      <div class="relative z-10 text-right">
        ...
        <h1 class="text-6xl font-black text-white leading-[1.2] mb-6">
          أنشئ <br/>
          <span class="text-[#3EFF8B]">حماية أقوى.</span>
        </h1>
        <p class="text-white/70 text-xl max-w-md leading-relaxed font-medium">
          أعد تعيين بيانات اعتمادك من خلال نظام الأمان المتقدم الخاص بنا. تأكد من أن كلمة المرور الجديدة قوية وفريدة.
        </p>
      </div>
      ...
      <div class="relative z-10">
        <div class="flex items-center gap-4 p-6 bg-white/5 backdrop-blur-md rounded-3xl border border-white/10 max-w-sm mr-0 ml-auto flex-row-reverse">
          <div class="w-12 h-12 rounded-2xl bg-[#3EFF8B] flex items-center justify-center flex-shrink-0">
            <ShieldCheckIcon class="h-7 w-7 text-[#04724D]" />
          </div>
          <div class="text-right">
            <p class="text-white font-bold tracking-tight">استعادة الوصول</p>
            <p class="text-white/50 text-xs text-left">Credential Reset in Progress</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="w-full lg:w-1/2 flex flex-col justify-center px-8 sm:px-16 lg:px-24 xl:px-32 bg-gray-50/50 order-1">
      ...
      <div class="max-w-md w-full mx-auto text-right">
        <h2 class="text-4xl font-black text-gray-900 mb-2 tracking-tight">إعادة تعيين الأمان</h2>
        <p class="text-gray-500 font-bold mb-10 text-lg">قم بتعيين كلمة المرور الجديدة لاستعادة الوصول إلى لوحة التحكم.</p>
        ...
        <form @submit.prevent="submit" class="space-y-6 text-right">
          ...
          <!-- Email Input -->
          <div class="space-y-2">
            <label class="text-sm font-black text-gray-400 uppercase tracking-widest mr-1">بريد الحساب</label>
            <div class="relative group">
              ...
              <input 
                ...
                required
                dir="ltr"
                class="block w-full pr-11 pl-4 py-4 bg-gray-100 border border-gray-200 rounded-2xl focus:ring-4 focus:ring-[#3EFF8B]/10 focus:border-[#04724D] transition-all font-bold text-gray-500 placeholder:text-gray-300 pointer-events-none text-left"
          ...
          <!-- New Password Input -->
          <div class="space-y-2">
            <label class="text-sm font-black text-gray-400 uppercase tracking-widest mr-1">كلمة المرور الجديدة</label>
            <div class="relative group">
              <div class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
                <LockClosedIcon class="h-5 w-5 text-gray-400 group-focus-within:text-[#04724D] transition-colors" />
              </div>
              <input 
                ...
                required
                dir="ltr"
                class="block w-full pr-11 pl-12 py-4 bg-white border border-gray-200 rounded-2xl focus:ring-4 focus:ring-[#3EFF8B]/10 focus:border-[#04724D] transition-all font-bold text-gray-900 placeholder:text-gray-300 text-left"
                placeholder="••••••••"
              >
              <button 
                type="button"
                @click="showPassword = !showPassword"
                class="absolute inset-y-0 left-0 pl-4 flex items-center text-gray-400 hover:text-gray-600"
              >
                <EyeIcon v-if="!showPassword" class="h-5 w-5" />
                <EyeSlashIcon v-else class="h-5 w-5" />
              </button>
            </div>
            <p v-if="errors.password" class="text-red-500 text-xs font-bold mr-1 animate-pulse">{{ errors.password }}</p>
          </div>

          <!-- Confirm Password Input -->
          <div class="space-y-2">
            <label class="text-sm font-black text-gray-400 uppercase tracking-widest mr-1">تأكيد كلمة المرور الجديدة</label>
            <div class="relative group">
              <div class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
                <ShieldCheckIcon class="h-5 w-5 text-gray-400 group-focus-within:text-[#04724D] transition-colors" />
              </div>
              <input 
