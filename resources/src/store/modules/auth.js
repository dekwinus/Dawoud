...
const getters = {
    ...
    notifs_alert: state => state.notifs, // Returns object now

    // New Getters
    count_quantity_alert: state => state.notifs ? (state.notifs.quantity_alert || 0) : 0,
    count_invoice_due_alert: state => state.notifs ? (state.notifs.invoice_due_alert || 0) : 0,
    count_expiry_alert: state => state.notifs ? (state.notifs.expiry_alert || 0) : 0,

    notifs_total: state => {
        if (!state.notifs) return 0;
        if (typeof state.notifs === 'number') return state.notifs; // Backward compatibility fallback
        return (state.notifs.quantity_alert || 0) +
            (state.notifs.invoice_due_alert || 0) +
            (state.notifs.expiry_alert || 0);
    },
...
const actions = {
    async refreshUserPermissions({ commit }, i18n) {
        try {
            const userAuth = await window.axios.get("get_user_auth");
            ...
            // Set RTL from database
            const rtl = user.rtl ?? false;
            commit('config/setRtl', rtl, { root: true });
    ...
    async loadAvailableLanguages({ commit }) {
        try {
            const response = await window.axios.get("/languages"); // must return: [{ name, locale, flag }]
    ...
    logout({ commit }) {
        window.axios.post('/logout', {}, { baseURL: '' }) // override the baseURL here
            .then(() => {
