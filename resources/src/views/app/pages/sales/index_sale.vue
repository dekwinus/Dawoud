            ...
            <table class="table table-hover table-bordered table-md">
              ...
              <tbody>
                ...
                <tr v-for="payment in payments">
                  ...
                  <td>
                    <span v-if="payment.status == 'approved'" class="badge badge-outline-success">{{$t('Approved')}}</span>
                    <span v-else-if="payment.status == 'rejected'" class="badge badge-outline-danger">{{$t('Rejected')}}</span>
                    <span v-else class="badge badge-outline-info">{{$t('Pending')}}</span>
                  </td>
                  <td>
                    <div role="group" aria-label="Basic example" class="btn-group">
                      <span
                        v-if="currentUserPermissions.includes('payment_sales_edit') && payment.status === 'pending'"
                        title="Approve"
                        class="btn btn-icon btn-success btn-sm"
                        @click="Approve_Payment(payment.id)"
                      >
                        <i class="i-Yes"></i>
                      </span>
                      <span
                        ...
                        @click="Reject_Payment(payment.id)"
                      >
                        <i class="i-Close"></i>
                      </span>
                      <span
    ...
    <b-modal hide-footer size="sm" scrollable id="Show_invoice" :title="$t('Invoice_POS')" @shown="renderZatcaQr">
      <div id="invoice-POS">
        <div style="max-width:400px;margin:0px auto">
          ...
          <!-- Layout 3 - A5 Invoice (Exact Replica) -->
          <div v-else>
...
export default {
  ...
  methods: {
    ...
    print_it() {
      ...
      a.document.write('<link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;500;600;700&display=swap" rel="stylesheet">');
    ...
    Approve_Payment(id) {
      this.$swal({
        ...
        cancelButtonText: this.$t("Delete_cancelButtonText") // Re-use cancel button text
      }).then(result => {
        if (result.value) {
          NProgress.start();
          NProgress.set(0.1);
          axios
            .post("payment_sale/" + id + "/approve")
            .then(() => {
              this.$swal(
                this.$t("Approved"),
