                  ...
                  <b-col lg="4" md="4" sm="12" v-if="payment.status != 'pending' && sale.statut == 'completed'">
                    <validation-provider name="Account">
                      <b-form-group slot-scope="{ valid, errors }" :label="$t('Account')">
                        <!-- Show badge if payment method has linked account -->
                        <div v-if="linkedAccountName" class="linked-account-badge">
                          <i class="i-Wallet" style="margin-right:6px;"></i>
                          {{ linkedAccountName }}
                        </div>
...
export default {
  ...
  computed: {
    ...
    linkedAccountName() {
      const pm = (this.payment_methods || []).find(m => String(m.id) === String(this.payment.payment_method_id));
      if (pm && pm.account_id) {
        const acct = (this.accounts || []).find(a => String(a.id) === String(pm.account_id));
        return acct ? (acct.account_name || acct.name) : '';
      }
      return '';
    },
  ...
  watch: {
    'payment.payment_method_id'(newVal) {
      const pm = (this.payment_methods || []).find(m => String(m.id) === String(newVal));
      if (pm && pm.account_id) {
        this.payment.account_id = pm.account_id;
      }
    },
    GrandTotal(val) {
      if (Number(val) < 0) {
  ...
  methods: {
    ...
    //---------------------- get_units ------------------------------\\
    // We utilize local data from the detail object now, but keep this for fallback if needed
    get_units(value) {
      axios
    ...
    Selected_Unit(value) {
      ...
      if (unit) {
        ...
        if (unit.operator == '/') {
          this.detail.stock = parseFloat((this.detail.fix_stock * unit.operator_value).toFixed(2));
        } else {
          this.detail.stock = parseFloat((this.detail.fix_stock / unit.operator_value).toFixed(2));
        }

        // Recalculate Price based on Base Price (fix_price)
        // If converting FROM base TO unit:
        // Price = BasePrice * OperatorValue (if operator is '*') OR BasePrice / OperatorValue (if operator is '/')
        // Wait, standard logic in this app seems to be:
        // If UnitOperator is '/': UnitPrice = BasePrice / Value
        // If UnitOperator is '*': UnitPrice = BasePrice * Value      
        if (unit.operator == '/') {
        ...
        // Update baselines for price type switching
        if(this.detail.price_type === 'wholesale'){
           this.detail.wholesale_unit_price = this.detail.Unit_price;
           // If we have a retail baseline, we should probably update it too? 
           // Best effort: approximate retail based on same ratio if possible, or just reset to current price
           // For simplicity, just set current.
        } else {
           this.detail.retail_unit_price = this.detail.Unit_price;
        }

        // Re-apply Tax & Discount
        if (this.detail.discount_Method == "2") {
          this.detail.DiscountNet = this.detail.discount;
        } else {
          this.detail.DiscountNet = parseFloat(
            (this.detail.Unit_price * this.detail.discount) / 100
          );
        }

        if (this.detail.tax_method == "1") {
            this.detail.Net_price = parseFloat(
              this.detail.Unit_price - this.detail.DiscountNet
            );
             this.detail.taxe = parseFloat(
    ...
    Modal_Updat_Detail(detail) {
      ...
      // If no units found (legacy data), fallback to API
    ...
    Get_Product_Details(product_id, variant_id) {
      axios.get("/show_product_data/" + product_id +"/"+ variant_id).then(response => {
        ...
        this.product.unit_data = response.data.unit_data;
        this.product.unit_sale_data = response.data.unit_sale_data;
