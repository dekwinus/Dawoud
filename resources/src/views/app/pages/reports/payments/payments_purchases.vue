        ...
        >
          <template slot="table-row" slot-scope="props">
            ...
            <span v-else-if="props.column.field === 'approval_status'">
              <span v-if="props.row.approval_status === 'approved'" class="badge badge-success">{{$t('Approved')}}</span>
              <span v-else class="badge badge-warning">{{$t('Pending')}}</span>
              <b-button
                v-if="props.row.approval_status !== 'approved' && currentUserPermissions.includes('payment_purchases_edit')"
                size="sm"
                variant="outline-success"
                class="ml-2 btn-pill"
                @click="Approve_Payment(props.row.id)"
              >
                <i class="i-Yes"></i> {{$t('Approve')}}
              </b-button>
            </span>
...
export default {
  ...
  computed: {
    ...
    columns() {
      return [
        ...
        { label: this.$t("ApprovalStatus"), field: "approval_status", tdClass:"text-left", thClass:"text-left", sortable:false }
      ];
    },
    ...
    excelColumns() {
      return [
        ...
        { label: this.$t("ApprovalStatus"), field: "approval_status" }
      ];
    },
  ...
  methods: {
    ...
    Approve_Payment(id) {
      this.$swal({
        title: this.$t("Approve_Title") || "Approve Payment?",
        text: this.$t("Approve_Text") || "This will apply the payment to accounting.",
        type: "warning",
        ...
        if (result.value) {
          ...
          NProgress.set(0.1);
          axios
            .post("payment_purchase/" + id + "/approve")
            .then(() => {
              setTimeout(() => NProgress.done(), 500);
              this.$swal(
                this.$t("Approved") || "Approved",
                this.$t("Payment_Approved_Successfully") || "Payment approved successfully",
                "success"
              );
              this.Payments_Purchases(this.serverParams.page);
            })
            .catch(() => {
              ...
              this.$swal(
                ...
                this.$t("Approve_Failed") || "Failed to approve payment",
                "warning"
              );
            });
        }
