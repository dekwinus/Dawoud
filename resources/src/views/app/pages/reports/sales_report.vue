...
components: { DateRangePicker },
  ...
  methods: {
    ...
    get_data_loaded() {
      ...
      // If we are filtering by "due_soon", we likely want to see ALL due invoices, not just from year 2000 or today
      // actually year 2000 is fine as a start date, but let's make sure we don't accidentally filter them out.
      // The backend filter logic applies ON TOP of date filter in original code?
      // "whereBetween('sales.date', [$request->from, $request->to])" is line 669.
      // So if the due invoice is older than "01/01/2000", it won't show. That's probably fine.
      // But if the user changes the date picker, it might hide them.
      // For now, let's leave the date logic as is, but maybe expand the range?
      // Actually, if filter is due_soon, we should probably ignore the date range in the backend OR ensure date range is wide enough.
      // Let's just trust the default wide range for now.
    },
