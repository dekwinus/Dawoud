             ...
             <b-col lg="6" md="6" sm="12">
              <validation-provider name="Account">
                <b-form-group slot-scope="{ valid, errors }" :label="$t('Account')">
                  <div v-if="linkedFactureAccountName" class="linked-account-badge">
                    <i class="i-Wallet" style="margin-right:6px;"></i>
                    {{ linkedFactureAccountName }}
                  </div>
                  <v-select
                    v-else
                    :class="{'is-invalid': !!errors.length}"
...
export default {
  ...
  computed: {
    ...
    linkedFactureAccountName() {
      const pm = (this.payment_methods || []).find(m => String(m.id) === String(this.facture.payment_method_id));
      if (pm && pm.account_id) {
  ...
  watch: {
    'facture.payment_method_id'(newVal) {
      const pm = (this.payment_methods || []).find(m => String(m.id) === String(newVal));
      if (pm && pm.account_id) {
        this.facture.account_id = pm.account_id;
      }
    }
  },
