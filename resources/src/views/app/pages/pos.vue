  ...
  <b-modal hide-footer size="sm" scrollable id="Show_invoice" :title="$t('Invoice_POS')" @shown="renderZatcaQrPos">
        <div id="invoice-POS">
          <div style="max-width:400px;margin:0px auto">
            ...
            <!-- Layout 3 - A5 Invoice (Refined Compact Version) -->
            <div v-else>
               <div style="width:100%; margin:0 auto; font-family: 'Cairo', sans-serif; direction: rtl; text-align: right; color: #000; font-size: 9px; line-height: 1.2;">
              
                <!-- Header -->
                <div style="text-align: center; margin-bottom: 2px; border-bottom: 1px solid #000; padding-bottom: 2px;">
                  <h1 style="margin: 0; font-weight: 900; font-size: 14px; color: #000; letter-spacing: 1px;">{{invoice_pos.setting.CompanyName}}</h1>
                </div>
                   ...
                   <table style="width: 100%; border: none;">
                       <tr style="border: none;">
                           <td style="border: none; text-align: right; width: 50%;">
                               <span style="font-weight: bold;">رقم الفاتورة:</span> {{invoice_pos.sale.Ref}} <br>
                               <span style="font-weight: bold;">التاريخ:</span> {{invoice_pos.sale.date}}
                           </td>
                           <td style="border: none; text-align: left; width: 50%;">
                               <span style="font-weight: bold;">المبيعات:</span> {{invoice_pos.sale.seller_name}}
                           </td>
                       </tr>
                   </table>
                </div>
              ...
              <table style="width: 100%; border-collapse: collapse; border: 1px solid #000; margin-bottom: 5px;">
                ...
                <tbody>
                  <tr v-for="(detail_invoice, index) in invoice_pos.details" :key="index">
              ...
              <!-- Totals Section (Single Row) -->
              <div style="border: 1px solid #000; padding: 2px; margin-bottom: 0;">
                  <table style="width: 100%; border: none;">
                      <tr style="border: none;">
                          <td style="border: none; text-align: center; width: 33%;">
                              <span style="font-weight: bold;">الاجمالي:</span> {{formatNumber(invoice_pos.sale.GrandTotal, 2)}}
                          </td>
                          <td style="border: none; text-align: center; width: 33%;">
                              <span style="font-weight: bold;">المدفوع:</span> {{formatNumber(invoice_pos.sale.paid_amount, 2)}}
                          </td>
                          <td style="border: none; text-align: center; width: 33%;">
                              <span style="font-weight: bold;">المتبقي:</span> {{formatNumber(invoice_pos.sale.GrandTotal - invoice_pos.sale.paid_amount, 2)}}
                          </td>
                      </tr>
                  </table>
              </div>
           ...
           <b-col lg="6" md="6" sm="12" v-if="detail.product_type != 'is_service'" v-show="!detailLoading">
            <validation-provider name="Unit Sale" :rules="{ required: true}">
              <b-form-group slot-scope="{ valid, errors }" :label="$t('UnitSale') + ' ' + '*'"><v-select
                  ...
                  :reduce="label => label.value"
                  @input="Selected_Unit"
                  :options="units.map(u => ({label: u.name || u.ShortName, value: u.id}))"
                />
    ...
    <div class="pos-footer-bar" v-if="productsReady">
      ...
      <div class="footer-main-group">
        ...
        <button 
          ...
          :disabled="paymentProcessing || details.length === 0 || (currentUserPermissions && !currentUserPermissions.includes('Sales_add'))" 
          :title="currentUserPermissions && !currentUserPermissions.includes('Sales_add') ? $t('AccessForbidden') : $t('pos.Complete_and_process_payment')"
        >
          <svg viewBox="0 0 24 24" fill="currentColor">
...
export default {
  ...
  computed: {
    ...
    // Add 'receipt-148' CSS class to body when printing if 148mm chosen
    currentReceiptPaperSizeClass() {
      ...
      if (size === 148 || this.currentReceiptLayout === 3) return 'receipt-148';
      return 'receipt-80';
    },
  ...
  methods: {
      ...
      if (anyNewCard) {
        ...
        axios
          ...
          .catch(error => {
            NProgress.done();
            this.paymentProcessing = false;
            let errorMsg = this.$t("InvalidData");
            if (error.response && error.response.data && error.response.data.message) {
              errorMsg = error.response.data.message;
              if (errorMsg === 'Credit Limit Exceeded') {
                errorMsg = this.$t('Credit_Limit_Exceeded');
              }
            }
    ...
    async Get_Product_Details(product_id, variant_id, sourceProduct = null) {
      ...
      const applyDetail = (data) => {
        ...
        this.product.unit_data          = data.unit_data;
        this.product.unit_sale_data     = data.unit_sale_data;
      ...
      const buildDetailFromSource = () => {
        ...
        const detail = {
          ...
          unit_data: p.unit_data,
          unit_sale_data: p.unit_sale_data,
          // Stock
    ...
    handleProductClick(product) {
      ...
      if (product.expiry_date) {
        ...
        if (product.expiry_date < today) {
           this.makeToast("danger", "Product is expired", "Error");
           return;
        }
      }
    ...
    Selected_Unit(value) {
      // Find the selected unit object from the available units
      let unit = this.units.find((u) => u.id === value);
      if (unit) {
        // Update unit name
        this.detail.unitSale = unit.ShortName;
        
        // Recalculate Stock based on Base Stock (fix_stock)
        // If operator is '/', Divide. If '*', Multiply. 
        // Logic: unit_qty = base_qty [op] value ?? 
        // Wait, standard logic in this app seems to be:
        // if '/' -> stock = fix_stock * val
        // if '*' -> stock = fix_stock / val
        // mirroring the Update_Detail logic at line 4542
        if (unit.operator == '/') {
          this.detail.current = parseFloat((this.detail.fix_stock * unit.operator_value).toFixed(2));
        } else {
          this.detail.current = parseFloat((this.detail.fix_stock / unit.operator_value).toFixed(2));
        }
        
        // Recalculate Price based on Base Price (fix_price)
        // Ensure we handle price updates correctly
        if (unit.operator == '/') {
            this.detail.Unit_price = parseFloat((this.detail.fix_price / unit.operator_value).toFixed(2));
        } else {
            this.detail.Unit_price = parseFloat((this.detail.fix_price * unit.operator_value).toFixed(2));
        }
        ...
        // Update baselines
        if(this.detail.price_type === 'wholesale'){
           this.detail.wholesale_unit_price = this.detail.Unit_price;
        } else {
           this.detail.retail_unit_price = this.detail.Unit_price;
        }
    ...
    Modal_Updat_Detail(detail) {
      ...
      this.detail.name = detail.name;
      this.$bvModal.show("form_Update_Detail");
      // Construct available units from local data
      this.units = [];
      if(detail.unit_data){
           this.units.push({
               id: detail.unit_data.id, 
               name: detail.unit_data.ShortName, 
               ShortName: detail.unit_data.ShortName, 
               operator: '/', 
               operator_value: 1 
           });
      }
      if(detail.unit_sale_data && detail.unit_sale_data.id !== (detail.unit_data ? detail.unit_data.id : null)){
          this.units.push(detail.unit_sale_data);
      }
      
      let promise;
      if(this.units.length > 0){
          promise = Promise.resolve(this.units);
      } else {
          promise = this.get_units(detail.product_id);
      }
    ...
    formatArabicNumber(number, dec) {
      const formatted = this.formatNumber(number, dec);
    ...
    print_pos(opts = {}) {
      ...
      var divContents = el.innerHTML;
          // If A5 or Layout 3, width needs to be wider (approx 148mm + margins => ~600px+)
      // Standard 80mm is usually printed at ~300-350px width window.
      // A5 is 148mm wide => ~560px at 96dpi. We can use 600px width window.
      const isA5 = (this.currentReceiptPaperSize === 148 || this.currentReceiptLayout === 3);
      const winWidth = isA5 ? 650 : 480;
      var a = window.open('', '', `height=600,width=${winWidth}`);
      if (!a) { return; }
      const bodyClass = this.currentReceiptPaperSizeClass || '';
      // Clone parent styles REMOVED to avoid size locking issues
      // Only include necessary styles
      
      // Force A5 size if applicable
      let a5Css = '';
      if (isA5) {
        a5Css = '@page { margin: 0; } body { width: 100% !important; margin: 0 !important; padding: 0 !important; min-height: 210mm; }';
      }
      ...
      a.document.write('<html><head><link rel="stylesheet" href="/css/pos_print.css">' + customStyle + '</head><body class="">');
    ...
    print_pos_mobile() {
      try {
        ...
        // Clone parent styles REMOVED
        
        // Force A5 size if applicable for mobile too
        const isA5 = (this.currentReceiptPaperSize === 148 || this.currentReceiptLayout === 3);
        let a5Css = '';
        if (isA5) {
            a5Css = '@page { margin: 0; } body { width: 100% !important; margin: 0 !important; padding: 0 !important; min-height: 210mm; }';
        ...
        const customStyle = `
            <style>
            @import url("https://fonts.googleapis.com/css2?family=Changa:wght@400;500;600;700;800;900&display=swap");
            #invoice-POS {
                width: 100% !important;
                max-width: 100% !important;
        ...
        const html = `<!doctype html><html><head>
          ...
          ${customStyle}
        </head><body class=""><div id="invoice-POS">${el.innerHTML}</div></body></html>`;
    ...
    async openModernPaymentModal() {
      ...
      // Re-fetch client data to ensure we have the latest credit limit and balance
      try {
        const briefResponse = await axios.get(`/clients/${this.selectedClientId}/brief`);
        const briefData = briefResponse.data;
        this.selectedClientCreditLimit = parseFloat(briefData.credit_limit || 0);
        this.selectedClientNetBalance = parseFloat(briefData.netBalance || 0);
      } catch (error) {
      ...
      // Open the modal via the child component ref
      if (this.$refs.modernPaymentModal) {
