             ...
             <b-col lg="12" md="6" sm="12">
              <validation-provider name="Account">
                <b-form-group slot-scope="{ valid, errors }" :label="$t('Account')">
                  <div v-if="linkedPaymentAccountName" class="linked-account-badge">
                    <i class="i-Wallet" style="margin-right:6px;"></i>
                    {{ linkedPaymentAccountName }}
                  </div>
                  <v-select
                    v-else
                    :class="{'is-invalid': !!errors.length}"
        ...
        <b-form @submit.prevent="Submit_Payment_sell_return_due">
          <b-row>
            ...
            <b-col lg="12" md="12" sm="12">
              <validation-provider name="Account">
                <b-form-group slot-scope="{ valid, errors }" :label="$t('Account')">
                  <div v-if="linkedReturnAccountName" class="linked-account-badge">
                    <i class="i-Wallet" style="margin-right:6px;"></i>
                    {{ linkedReturnAccountName }}
                  </div>
                  <v-select
                    v-else
                    :class="{'is-invalid': !!errors.length}"
...
export default {
  ...
  computed: {
    ...
    linkedReturnAccountName() {
      const pm = (this.payment_methods || []).find(m => String(m.id) === String(this.payment_return.payment_method_id));
      if (pm && pm.account_id) {
        const acct = (this.accounts || []).find(a => String(a.id) === String(pm.account_id));
        return acct ? (acct.account_name || acct.name) : '';
      }
  ...
  watch: {
    ...
    'payment_return.payment_method_id'(newVal) {
      const pm = (this.payment_methods || []).find(m => String(m.id) === String(newVal));
      if (pm && pm.account_id) {
        this.payment_return.account_id = pm.account_id;
      }
    }
  },
