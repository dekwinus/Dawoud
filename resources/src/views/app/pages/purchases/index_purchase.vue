            ...
            <table class="table table-hover table-bordered table-md">
              <thead>
                <tr>
                  ...
                  <th scope="col">{{$t('ApprovalStatus')}}</th>
              ...
              <tbody>
                ...
                <tr v-for="facture in factures">
                  ...
                  <td>
                    <span v-if="facture.approval_status === 'approved'" class="badge badge-success">{{$t('Approved')}}</span>
                    <span v-else class="badge badge-warning">{{$t('Pending')}}</span>
                  </td>
                  <td>
                    <div role="group" aria-label="Basic example" class="btn-group">
                      <span
                        v-if="facture.approval_status !== 'approved' && currentUserPermissions.includes('payment_purchases_edit')"
                        :title="$t('Approve')"
                        class="btn btn-icon btn-outline-success btn-sm"
                        @click="Approve_Payment(facture.id)"
                      >
                        <i class="i-Yes"></i>
                      </span>
                      <span
...
export default {
        ...
        if (result.value) {
          ...
          axios
            ...
            .then(() => {
              ...
              Fire.$emit("Approve_Facture_purchase");
            })
            .catch(() => {
              setTimeout(() => NProgress.done(), 500);
              this.$swal(
  ...
  created: function() {
    ...
    Fire.$on("Approve_Facture_purchase", () => {
      setTimeout(() => {
        NProgress.done();
        this.$bvModal.hide("Show_payment");
        this.Get_Purchases(this.serverParams.page);
