                  ...
                  <div class="payment-lines-list">
                    <div v-for="(p, idx) in paymentLines" :key="idx" class="payment-line-card">
                      ...
                      <div class="payment-line-body">
                        ...
                        <div class="input-field" v-if="!getLinkedAccountId(p)">
                        ...
                        <div class="input-field" v-else>
                          <label class="field-label">{{$t('Account')}}</label>
                          <div class="linked-account-badge">
                            <i class="i-Wallet" style="margin-right:6px;"></i>
                            {{ getLinkedAccountName(p) }}
                          </div>
                        </div>
                      <div class="form-group">
...
export default {
  ...
  methods: {
    ...
    isCreditCardMethod(method) {
      // Inistapay (previously ID 1) is NOT a credit card flow in this system anymore.
      // E-Wallet (previously Check) is also not a card.
      if (!method || !method.name) return false;
      const name = method.name.toLowerCase();
      // Explicitly exclude Inistapay from card flow logic
      if (name.includes('inistapay')) return false;
    ...
    changePaymentMethod(line, method) {
      ...
      // Dynamic Accounting: auto-set accountId from linked account
      if (method.account_id) {
        line.accountId = method.account_id;
      }
    ...
    getLinkedAccountId(line) {
      const pm = (this.resolvedPaymentMethods || []).find(m => String(m.id) === String(line.paymentMethodId));
      return pm && pm.account_id ? pm.account_id : null;
    },
    getLinkedAccountName(line) {
      const accountId = this.getLinkedAccountId(line);
      if (!accountId) return '';
      const account = (this.resolvedAccounts || []).find(a => String(a.id) === String(accountId));
      return account ? (account.account_name || account.name) : '';
    },
    getPaymentIcon(method) {
      const name = ((method && method.name) || '').toLowerCase();
      if (name.includes('cash')) return 'i-Money-Bag';
      if (name.includes('inistapay')) return 'i-Financial'; // Use a suitable icon
      if (name.includes('e-wallet') || name.includes('wallet')) return 'i-Wallet';
    ...
    getCashMethodId() {
      ...
      // Fallback: return the first available method if 'cash' not found
    ...
    addPaymentLine() {
      ...
      if (!defaultMethodId) {
         this.makeToast('danger', this.$t ? this.$t('NoPaymentMethodFound') : 'No payment methods available', this.$t ? this.$t('Failed') : 'Failed');
         return;
      }
    ...
    async processPayment() {
      ...
      try {
        ...
        let errorMsg = this.$t ? this.$t('InvalidData') : 'Invalid data';
        if (e.response && e.response.data && e.response.data.message) {
          errorMsg = e.response.data.message;
          // If it's a credit limit error, try to translate or format it
          if (errorMsg === 'Credit Limit Exceeded' && this.$t) {
            errorMsg = this.$t('Credit_Limit_Exceeded');
          }
        }
        this.makeToast('danger', errorMsg, this.$t ? this.$t('Failed') : 'Failed');
      }
    },
    ...
    async CreatePOS() {
      ...
      try {
        ...
        if (isNetworkError) {
          // Use a distinctive message to verify the fix is active
          let msg = 'Network connection failed (ERR_POS_NET).';
          if (error.config && error.config.url) {
            msg += ' URL: ' + error.config.url;
          }
          if (this.$t) {
            const translated = this.$t('pos.NetworkError');
            if (translated && translated !== 'pos.NetworkError' && translated !== 'POS.NETWORKERR') {
              msg = translated + (error.config && error.config.url ? (' (' + error.config.url + ')') : '');
            }
          }
